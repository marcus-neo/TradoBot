...
class Generator:
    ...
    def _classify(self, preprocessed_data):
        ...
        for index, _ in enumerate(close):
            if index >= self.prediction_length:
                offset = 1
                position = 0
                while (
                    ...
                    if (
                        low[index + offset]
                        <= (1 - self.successful_trade_percent) * close[index]
                    ):
                        position = -1
                        break
                    if (
        ...
        shaved_output = output[self.prediction_length : pos]  # noqa: E203
        return (
            shaved_output,
            (
    ...
    def max_indicator_length(self):
        ...
        for _, indicator_params in self.kwargs:
            if "period_list" in indicator_params:
                max_length = max(
                    max_length, max(indicator_params["period_list"])
                )
        return max_length
...
class GenerateTest(Generator):
    ...
    """Class to generate the test dataset."""
    ...
    def _create_df(self, preprocessed_data, dataframe):
        ...
        normalized_data = _normalize_dataframe(processed_data)
...
class GenerateTrain(Generator):
    ...
    """Class to generate the training dataset."""
